    ...
    try {
        ...
        if (!s_id) {
            ...
            return;
             ...
             const response = await fetch(`http://localhost:8080/api/complaint/processing/${s_id}`);
                    ...
                    <p>Reported         : ${new Date(complaint.postedAt).toLocaleString()}</p>
                        ...
                        updateButton.addEventListener("click", () => {
                           ...
                           // statusButton.className = `status-label ${statusClass}`; // Update class dynamically if needed
                            ...
                            fetch(`http://localhost:8080/api/complaint/update/resolved/${complaint.complaint_id}`, {
                                method: "POST",
                                headers: {
